...
def main():
    ...
    # P[0,1] = α (probability 0 → 1)
    # P[1,0] = α (probability 1 → 0)
    alpha, beta = 0.3, 0.3
    ...
    try:
        ...
        # Prepare |�_�� = �[0]|00� + �[1]|11� (simplified)
    ...
    # Prepare computational basis state |01� (edge 0�1)
    print("Preparing computational basis state |01�...")
    basis_circuit = prepare_basis_state(index=1, num_qubits=2)
    print(f" Basis state |01� prepared")
    print()
    ...
    fig2 = plot_qpe_histogram(qpe_basis, analysis_basis, 
                             figsize=(10, 6), show_phases=True)
    plt.title("QPE Results: Computational Basis State |01�")
    plt.tight_layout()
    ...
    print(f"Classical mixing time bound (�=0.01): {classical_mixing}")
...
def create_summary_plot(P: np.ndarray, pi: np.ndarray, 
    ...
    ax.bar(x + width/2, probs_basis, width, label='Basis State |01�', 
